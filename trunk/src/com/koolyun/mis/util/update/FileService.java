...
public class FileService {
	...
	public Map<Integer, Integer> getData(String path, String version) {
		...
		Cursor cursor = db.rawQuery("select threadid, downlength from filedownlog where downpath=? " + "and version=?",
				new String[] { path, version });
	 ...
	 * @param map
	 */
	public void save(String path, Map<Integer, Integer> map, String version) {// int
																				// threadid,
																				// int
																				// position
	...
	public void update(String path, int threadId, int pos, String version) {
		SQLiteDatabase db = openHelper.getWritableDatabase();
		db.execSQL("update filedownlog set downlength=? where downpath=? and threadid=? and version=?", new Object[] {
				pos, path, threadId, version });
		db.close();
	}
